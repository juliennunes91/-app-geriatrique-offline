...
- **Le rapport le trace, mais sans le crier.** Une prescription relue et maintenue en
  connaissance de cause n'appelle plus d'action : la rendre avec la même insistance rouge
  que le reste fait paraître le rapport plus alarmant qu'il ne l'est, et noie ce qui reste
  vraiment à trancher. L'entrée sort donc du **corps** de la section « prescriptions
  inappropriées » pour un relevé calme placé à sa **fin** — « Assumé par le prescripteur »,
  une ligne par décision, titre + motif, en gris. Même forme que « Points de méthode ».
  Sans cette trace, un lecteur tiers verrait une couleur adoucie sans explication.
- **Trois écueils traversés avant d'y arriver**, tous mesurés sur un rapport réel : le bloc
  n'affichait que la **clé** de l'alerte (« EV_D21 »), identifiant interne muet pour le
  destinataire — le titre est désormais retrouvé dans le DOM rendu par la clé de masquage,
  un seul chemin pour les quatre familles ; il était placé **en tête** de rapport alors que
  les points figurent plus bas ; et un relevé **séparé** les répétait une troisième fois,
  laissant le lecteur rapprocher lui-même deux listes. L'icône de gradation et le point
  final, hérités de l'écran, sont retirés avant d'accoler le motif.
